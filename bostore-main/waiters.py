...
class Waiters:
    ...
    def bind_client(self, client: pyrogram.Client):
        ...
        self.client.add_handler(
            pyrogram.handlers.MessageHandler(self.message_handler), -1
        )
    ...
    async def message_handler(
        ...
        for waiter_obj in self.handlers:
            if user.waiting_for != waiter_obj["waiting_for"] or not (
                await waiter_obj["filters"](client, message)
                if callable(waiter_obj["filters"])
                else True
            ):
                continue
    ...
    def add_handler(self, waiting_for: str, filters=None):
        def decorator(func):
            self.handlers.append(
                {"waiting_for": waiting_for, "callback": func, "filters": filters}
            )
            return func
